  ...
  document.querySelectorAll('[data-lang-key]').forEach(element => {
    ...
    if (translations[key]) {
      if (element.tagName === 'H1' || element.tagName === 'H2' || element.tagName === 'P') {
        element.innerHTML = translations[key]; // for innerHTML to allow spans
      } else {
        element.innerText = translations[key]; // for plain text
      }
    }
  });
}
...
function switchLanguage(lang) {
  ...
  fetch('path/to/language.json')
    .then(response => response.json())
    .then(data => updateLanguage(data[lang]));
}
  ...
  function calculateAge() {
    ...
    const birthMonth = 6;  // July (0-based, so January is 0, July is 6)
    const birthDay = 2;
    ...
    // If today's date is before July 2nd, subtract 1 from the age
...
// Call displayAge on page load to display the correct age
